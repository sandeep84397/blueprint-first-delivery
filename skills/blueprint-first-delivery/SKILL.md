...
2. Classify each proposed chunk as independent, ordered, or integration-only. A consumer of another chunk's type, contract, schema, or behavior is ordered. Mark work parallel only when contracts are frozen **and** file/state ownership does not overlap; otherwise sequence it.
...
6. After every component-level gate passes, execute the separate integration blueprint and then run its separate integration gate. Unit tests alone never satisfy integration.
